/*!
 * ZetPay.js — embed ZetPay checkout on your own website
 * Full docs: Developer Portal -> API Integration (api.html) in your ZetPay dashboard.
 *
 * Usage:
 ...
 * failure mode. If ZetPay's backend ever moves to a new domain, update
 * the single constant below (or use the data-api override).
 */
(function (global) {
  // ZetPay's backend. This is intentionally NOT derived from where this
  ...
  function getApiBase() {
    ...
    for (let i = 0; i < scripts.length; i++) {
      const s = scripts[i];
      if (s.src && (s.src.indexOf('zappay-pay.js') !== -1 || s.src.indexOf('zetpay-pay.js') !== -1) && s.dataset && s.dataset.api) {
  ...
  const ZetPay = {
    ...
    /**
     * Create an order and open the ZetPay checkout as a popup.
     ...
     * fell back to ZetPay's OWN site as the redirect target — so a paid
    ...
    async createOrder(opts) {
      opts = opts || {};
      if (!opts.zap_api) { console.error('ZetPay.js: zap_api key is required'); return; }
      if (!opts.amount) { console.error('ZetPay.js: amount is required'); return; }
      ...
      try {
        ...
        if (!data.success) {
          console.error('ZetPay.js:', data.message);
          callbacks.onFailed && callbacks.onFailed({ error: data.message });
          return;
        }

        const { order_id, payment_url, amount } = data.data;
        popupRef = window.open(payment_url, 'ZetPayCheckout', 'width=430,height=700,menubar=no,toolbar=no');
        if (!popupRef) {
        ...
        console.error('ZetPay.js: network error creating order', e);
        callbacks.onFailed && callbacks.onFailed({ error: 'network_error' });
      }
    },
  };

  global.ZapPay = ZetPay;
  global.ZetPay = ZetPay;
})(window);
